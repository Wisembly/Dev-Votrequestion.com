...
class dbal_mysql extends dbal
{
	...
	function sql_connect($sqlserver, $sqluser, $sqlpassword, $database, $port = false, $persistency = false, $new_link = false)
	{
		...
		if ($this->db_connect_id && $this->dbname != '')
		{
			if (@mysql_select_db($this->dbname, $this->db_connect_id))
			{
				...
				$recup_server_info = $this->sql_server_info(true);
				if (version_compare($recup_server_info, '4.1.0', '>='))
				{
					mysql_query("SET NAMES 'utf8'");

					// enforce strict mode on databases that support it
					if (version_compare($recup_server_info, '5.0.2', '>='))
					{
						if ( !isset($db_sql_modes) )
						{
							...
							$modes = array_map('trim', explode(',', $db_sql_modes));
						...
						$mode = implode(',', $modes);
						@mysql_query("SET SESSION sql_mode='{$mode}'");
					}
				}
	...
	*/
	...
	{
		global $cache, $db_sql_version;

		if ( isset($db_sql_version) && !empty($db_sql_version) )
		{
			$this->sql_server_version = $db_sql_version ;
		...
		switch ($mode)
		{
			case 'start':
				...
				if (preg_match('/^SELECT/', $explain_query))
				{
					...
					if ($result = mysql_query("EXPLAIN $explain_query", '@'))
					{
					...
					if ($test_prof)
					{
						...
						if ($result = mysql_query('SHOW PROFILE ALL;', '@'))
						{
							$this->html_hold .= '<br />';
							while ($row = @mysql_fetch_assoc($result))
							{
